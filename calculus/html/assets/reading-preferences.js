(() => {
  ...
  const textSelector = [
    ...
    '.pnm','.pnm *','[class*="fcm-"]','[class*="mcm-"]','[class*="div-iv-"]','[class*="pbg-"]',
  ...
  const course = path.includes('/calculus/') ? 'calculus' : null;
  ...
  document.querySelectorAll('.portal-home-link').forEach(function(link) {
    if (!course) return;
    ...
    link.textContent = formulaDetail ? '返回公式摘要' : '返回學習中心 · 微積分';
  });

  var factorMap = { small: 0.88, medium: 1, large: 1.18 };

  var scaleElement = function(el) {
    var factor = factorMap[state.font] || 1;
  ...
  // Watch for dynamically added content (interactive models, etc.)
  var observer;
  ...
  var apply = function() {
    ...
    panel.querySelectorAll('[data-pref]').forEach(function(b) { b.setAttribute('aria-pressed', String(b.dataset.prefValue === state[b.dataset.pref])); });
    updateCompleteButton();
  };

  var close = function() { panel.hidden = true; toggle.setAttribute('aria-expanded', 'false'); toggle.focus(); };
  toggle.onclick = function() { panel.hidden = !panel.hidden; toggle.setAttribute('aria-expanded', String(!panel.hidden)); if (!panel.hidden) panel.focus(); };
  panel.addEventListener('click', function(e) {
    var b = e.target.closest('[data-pref]');
    if (b) { state[b.dataset.pref] = b.dataset.prefValue; apply(); }
    if (e.target.closest('.reader-reset')) { state = Object.assign({}, defaults); apply(); }
    if (e.target.closest('.reader-complete')) { var pages = completedPages(); var i = pages.indexOf(location.pathname); if (i >= 0) pages.splice(i, 1); else pages.push(location.pathname); try { localStorage.setItem('learning-portal-completed-pages', JSON.stringify(pages)); } catch(e) {} updateCompleteButton(); }
  });
  document.addEventListener('pointerdown', function(e) { if (!panel.hidden && !panel.contains(e.target) && !toggle.contains(e.target)) close(); });
