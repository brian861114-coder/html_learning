...
/* ── Legacy onclick support (transitional; per agent.md §12, migrate to data-action) ── */
...
window.toggleTheme = function () {
  ...
  if (btn) {
    btn.textContent = next === "dark" ? "☀️ 淺色模式" : "🌙 深色模式";
  }
  try { localStorage.setItem("theme", next); } catch (_) {}
};

// Bind theme button via event listener (no inline onclick per agent.md §12)
const themeBtnEl = document.getElementById("themeBtn");
if (themeBtnEl) {
  themeBtnEl.addEventListener("click", () => window.toggleTheme());
}
